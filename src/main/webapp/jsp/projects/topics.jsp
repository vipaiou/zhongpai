...
<li><a class="ui-tab-current" href="<%=request.getContextPath()%>/projects/${project.id}/topics/">话题<span id="posts_count">${topicnum }</span></a></li>
...
<div id="topic_${topic.id }" class="timeline-box">
<div class="timeline-box-right">
<div class="timeline-box-title">
<div class="comment-title-avatar">
	<a target="_blank" href="<%=request.getContextPath() %>/user/${topic.userid}">
<img src="${imagehost }avatar-medium-${topic.avatar}"></a>
</div>
<div class="comment-title-avatar-r">
<p><a href="<%=request.getContextPath() %>/projects/${project.id }/topic/${topic.id }">
	<c:if test="${topic.type=='0' }">
		[<a href="<%=request.getContextPath()%>/projects/topics/list?filter=0&limit=40&project_id=${topic.projectId}" data-remote="true">普通话题</a>]
	</c:if>
	<c:if test="${topic.type=='1' }">
		[<a href="<%=request.getContextPath()%>/projects/topics/list?filter=0&limit=40&project_id=${topic.projectId}" data-remote="true">公告</a>]
	</c:if>
	<c:if test="${topic.type=='2' }">
		[<a href="<%=request.getContextPath()%>/projects/topics/list?filter=0&limit=40&project_id=${topic.projectId}" data-remote="true">常见问答</a>]
	</c:if>
	<c:if test="${topic.type=='3' }">
		[<a href="<%=request.getContextPath()%>/projects/topics/list?filter=0&limit=40&project_id=${topic.projectId}" data-remote="true">问题</a>]
	</c:if>
...
<c:if test="${project.createuser == topic.userid}"><a class="icon-user-f" href="#">项目发起人</a></c:if>
<a title="私信给 ${topic.username}" class="ui-popup-message post-private-letter" href="<%=request.getContextPath() %>/messages?recipient_id=${topic.userid}">发私信</a>
</span>
...
<div class="timeline-box-explain-text"><p>${topic.content }</p></div>
</div>
...
<p>
<a href="<%=request.getContextPath() %>/projects/${project.id }/topic/${topic.id }">${topic.commentnum }条回应，查看全文 &gt;&gt;</a>
<span data-visited-id="${topic.id}" data-visited-time="${fn:substring(topic.createdate,0,19) } +0800" class="list-icon-new" style="display: none;">新</span>
</p>
</div>
</div>
</div>
...
<td align="center"><a href="<%=request.getContextPath()%>/projects/${project.id }/topic/${topic.id }" class="c5">${topic.commentnum }</a></td>
<td class="timeline-posted-at" align="right"><a href="<%=request.getContextPath()%>/projects/${project.id }/topic/${topic.id }?latest=1" data-timestamp="${fn:substring(topic.createdate,0,19) } +0800"></a></td>
</tr>
</c:forEach>
</tbody>
</table>
